defmodule ReqLlmNext.Context do
  @moduledoc """
  ...
  @doc """
  Normalize any "prompt-ish" input into a validated ReqLlmNext.Context.
  ...
  def with_image(role, text_content, url, meta \\ %{}) do
    %Message{
      role: role,
      content: [ContentPart.text(text_content), ContentPart.image_url(url)],
      metadata: meta
    }
  end
  ...
  @doc """
  Executes tool calls and appends results to the context.

  For each tool call, finds the matching tool by name, executes it with the parsed
  arguments, and appends a tool result message to the context.

  ## Parameters

    * `context` - Current conversation context
    * `tool_calls` - List of ToolCall structs from an assistant message
    * `available_tools` - List of Tool structs with callbacks
  ...
  ## Examples
      ...
      tool_calls = [%ToolCall{id: "call_1", function: %{name: "add", arguments: ~s({"a":2,"b":3})}}]
      context = Context.execute_and_append_tools(context, tool_calls, tools)

  """
  @spec execute_and_append_tools(t(), [ToolCall.t()], [ReqLlmNext.Tool.t()]) :: t()
  ...
  defp extract_tool_call_info(%ToolCall{id: id, function: %{name: name}}), do: {name, id}
  defp extract_tool_call_info(%{name: name, id: id}), do: {name, id}

  defp find_and_execute_tool(
         %ToolCall{function: %{name: name, arguments: args_json}},
         available_tools
       ) do
  ...
  defp execute_tool_by_name(name, args, available_tools) do
    case Enum.find(available_tools, fn tool -> tool.name == name end) do
      ...
      tool ->
        ReqLlmNext.Tool.execute(tool, args)
    end
  end
  ...
  @doc "Bang version of validate/1; raises on invalid context."
  @spec validate!(t()) :: t()
  ...
  defimpl Enumerable do
    def count(%ReqLlmNext.Context{messages: messages}), do: {:ok, length(messages)}

    def member?(%ReqLlmNext.Context{messages: messages}, element) do
      {:ok, Enum.member?(messages, element)}
    end

    def reduce(%ReqLlmNext.Context{messages: messages}, acc, fun) do
  ...
  defimpl Collectable do
    def into(%ReqLlmNext.Context{messages: messages}) do
      collector = fn
        list, {:cont, message} -> [message | list]
        list, :done -> %ReqLlmNext.Context{messages: messages ++ Enum.reverse(list)}
        _list, :halt -> :ok
      end
